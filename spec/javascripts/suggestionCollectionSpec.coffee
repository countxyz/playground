...
describe 'SuggestionCollection', ->
  ...
  context 'with 10 mock suggestions', ->
    ...
    describe '#allBlured', ->
      ...
      it 'is true if all the suggestions are blured', ->
        collection.blurAll()
        expect( collection.allBlured() ).toBeTruthy()

      it 'is false if any suggestion is focused', ->
        collection.focus( 1 )
        expect( collection.allBlured() ).toBeFalsy()
